...
def get_graph_from_image(image, desired_nodes=75):
    ...
    for node in nodes:
        if nodes[node]["rgb_list"]:
            ...
            rgb_mean = np.zeros(3)  # Default value or handle differently if needed

        if nodes[node]["pos_list"]:
            nodes[node]["pos_list"] = np.stack(nodes[node]["pos_list"])
            pos_mean = np.mean(nodes[node]["pos_list"], axis=0)
        else:
            pos_mean = np.zeros(2)  # Default value or handle differently if needed

        # Ensure the features vector matches the expected NUM_FEATURES size
        features = np.concatenate(
            [
                np.reshape(rgb_mean, -1),
                np.reshape(pos_mean, -1)
            ]
        )[:NUM_FEATURES]  # Trim or pad to NUM_FEATURES size
